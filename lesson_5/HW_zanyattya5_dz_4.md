...
1) Написати task automatic з іменем check_count, який приймає
   ...
   кроку (input ??? name).
   Якщо counter.v написаний класичним Verilog (не
   SystemVerilog), для testbench рядок передається
   як вектор фіксованої ширини, наприклад
   input [8*10-1:0] name;  // "рядок" фіксованої ширини
   Ширина [8*10-1:0] має вміщати НАЙДОВШИЙ рядок
   Якщо пишете counter.sv у SystemVerilog  —
   тоді для testbench передається тип string:
3) Тіло task: порівняти count з expected (===), вивести
   PASS/FAIL через $display із назвою кроку.
4) Замінити ручні перевірки з пунктів 3-7 на виклики
